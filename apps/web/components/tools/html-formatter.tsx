...
export default function HtmlFormatter() {
  ...
  const format = () => {
    ...
    try {
      ...
      formatted = lines.map((line, i) => {
        ...
        const padding = indentStr.repeat(Math.max(0, indent));
        const formattedLine = padding + '<' + line + '>';
        if (line.match(/^<?\w[^>]*[^\/]$/) && !line.startsWith('input') && !line.startsWith('img') && !line.startsWith('br') && !line.startsWith('hr') && !line.startsWith('meta') && !line.startsWith('link')) indent++;
      ...
      toast.error('Error formatting HTML: ' + (error as Error).message);
      setOutput('');
    }
  };
  ...
  return (
    <ToolBase
      ...
      icon="📝"
      helpText="Format and beautify your HTML code with proper indentation. Also includes minify option to compress HTML."
      tips={[
        'Formats HTML with proper indentation',
        'Adjustable indent size (spaces)',
        'Minify option to compress HTML',
