...
export default function YamlFormatter() {
  ...
  const format = () => {
    if (!input.trim()) {
      toast.warning('Please enter YAML code first');
      return;
    }
    ...
    try {
      ...
      toast.error('Error formatting YAML: ' + (error as Error).message);
      setOutput('');
    }
  };
  ...
  const minify = () => {
    ...
    try {
      ...
      toast.error('Error minifying YAML: ' + (error as Error).message);
    }
  };
  ...
  return (
    <ToolBase
      ...
      description="Format and beautify YAML code"
      icon="📝"
      helpText="Format and beautify your YAML code with proper indentation. Also includes minify option to compress YAML."
      tips={[
        'Formats YAML with proper indentation',
        'Adjustable indent size (spaces)',
        'Minify option to compress YAML',
